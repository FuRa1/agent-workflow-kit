...
import { install, desiredFiles } from './install.mjs';
...
function doctor(root) {
  ...
  if (lock === null) errors.push('Missing managed lock');
  else validateLock(root, lock);
  const required = Object.keys(desiredFiles(root, config)).filter(relative =>
    !['.workflow-kit/project.json', 'design-polish.project.json'].includes(relative));
  for (const relative of required) {
    if (!Object.hasOwn(lock?.files ?? {}, relative)) errors.push(`Untracked required file: ${relative}`);
  }
  for (const [relative, expected] of Object.entries(lock?.files ?? {})) {
    if (!required.includes(relative)) warnings.push(`Stale managed path (not in current configuration/bundle): ${relative}`);
    const file = safePath(root, relative);
    if (!fs.existsSync(file)) errors.push(`Missing managed file: ${relative}`);
    else if (!fs.statSync(file).isFile()) errors.push(`Managed path is not a file: ${relative}`);
